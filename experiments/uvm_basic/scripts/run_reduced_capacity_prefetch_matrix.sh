...
if data.get("status") != "PASS_STAGE4_REDUCED_CAPACITY_CALIBRATION":
...
for ratio in 0.95 1.05 1.10; do
    for policy in "${POLICIES[@]}"; do
        for index in 1 2 3; do
            ...
            set -e
            if ((rc != 0)); then
                latest_exit="$(stage4_latest_case_exit "${STAGE4_RESULTS}/reduced_capacity/${policy}/${ratio}" 2>/dev/null || true)"
                if [[ "${policy}" == prefetch_none && "${TARGET}" == 8G && "${FALLBACK_USED}" == 0 &&
                      "${latest_exit}" == 124 ]]; then
                    printf 'prefetch_none timed out at 8G; restarting the matrix once at 6G.\n' \
                        >"${STAGE4_RESULTS}/PREFETCH_NONE_8G_TIMEOUT.txt"
                    STAGE4_TARGET_EFFECTIVE=6G \
                        bash "$(dirname "$0")/run_reduced_capacity_calibration.sh"
                    STAGE4_TARGET_EFFECTIVE=6G STAGE4_FALLBACK_USED=1 exec bash "$0"
                fi
...
counts = Counter()
safe = True
for path in root.glob("reduced_capacity/*/*/*/manifest.json"):
