...
| Transition validator | [Five-round approved plan](experiment/revision-transition-validator/plan-review.md), [Phase A result](experiment/revision-transition-validator/phase-a-results.md), [Phase B scheduler result](experiment/revision-transition-validator/phase-b-scheduler-results.md), and [Phase B prefetch result](experiment/revision-transition-validator/phase-b-prefetch-results.md). Scheduler and UVM prefetch production integration are ported across 575 and 610; host tests pass at 12 cases/145 assertions, all five modules build on each line, and the 610/7.1 module BTF exposes the typed scheduler and prefetch kfuncs without the raw migration function. Five exact-ABI scheduler verifier-load fixtures build and passed independent harness review, but have not run against a loaded custom module. Disposition remains `PARTIAL`: actual verifier outcomes, PMM membership/native tests, two PMM load fixtures, and live validation are open. |
...
| R5: safety and design depth | Executable verifier/transition rejection cases plus a source-backed re-count of the 50 agent safety events. | Demonstrate lane-varying, loop/bounds, overflow, stale, and conflicting-transition handling without claiming full-stack formal verification. | Reviewed `PARTIAL`: five full-pipeline verifier tests pass (28 targeted assertions; full suite 137/23), covering seven unsafe/control pairs. Scheduler and UVM prefetch now consume the shared transition validator on both 575 and 610, and the 610/7.1 build has the required module BTF. Five scheduler direct-write/admission fixtures build and passed independent harness review, but have not run against a loaded custom module. PMM production integration remains open. The 50-row inventory remains 46 `SUPPORTED` plus 4 `PARTIAL`; original sessions are absent. |
...
both driver lines. Five scheduler verifier-load fixtures are implemented and
independently reviewed, but their real load outcomes remain open. The UVM
prefetch action/coordinate path is also implemented, ported, built, and
reviewed offline; PMM membership/commit and kernel-native evidence remain open.
The NVBit 575
