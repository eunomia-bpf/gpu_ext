...
| Transition validator | [Five-round approved plan](experiment/revision-transition-validator/plan-review.md), [Phase A result](experiment/revision-transition-validator/phase-a-results.md), and [Phase B scheduler result](experiment/revision-transition-validator/phase-b-scheduler-results.md). Scheduler production integration is ported across 575 and 610; host tests pass at 12 cases/145 assertions, both BPF applications compile, all five modules build on each line, and the 610/7.1 module exposes the expected BTF types and setter kfuncs. Five exact-ABI verifier-load fixtures now build and passed independent harness review, but have not run against a loaded custom module. Disposition remains `PARTIAL`: actual verifier outcomes, UVM prefetch integration, PMM membership/native tests, and live validation are open. |
...
| R5: safety and design depth | Executable verifier/transition rejection cases plus a source-backed re-count of the 50 agent safety events. | Demonstrate lane-varying, loop/bounds, overflow, stale, and conflicting-transition handling without claiming full-stack formal verification. | Reviewed `PARTIAL`: five full-pipeline verifier tests pass (28 targeted assertions; full suite 137/23), covering seven unsafe/control pairs. The scheduler now consumes the shared transition validator on both 575 and 610, and the 610/7.1 build has the required module BTF. Its five exact-ABI direct-write/admission fixtures build and passed independent harness review, but have not run against a loaded custom module. UVM prefetch and PMM production integration remain open. The 50-row inventory remains 46 `SUPPORTED` plus 4 `PARTIAL`; original sessions are absent. |
...
independently reviewed, but their real load outcomes remain open. Phase B must
also connect the UVM paths and add kernel-native evidence. The NVBit 575
