#!/usr/bin/env python3
"""Five rotating soa-warp/SoA/baseline comparisons for the warp-contiguous
storage index (full-record-device-buffer LAYOUT=soa-warp).

Copy this file into the new campaign raw directory
(revision-rq4/raw/full-record-soa-warp-*), next to the copied
warp-contiguous-SoA collector binary renamed kernelretsnoop, then run it
from there. All 15 cells are new: no completed cell of the older
SoA/AoS/AoSoA campaigns is overwritten or rerun, and the frozen SoA arm
binary is reused, not rebuilt.

Arms:
  soa-warp - 32-bank field-major SoA GPU-local full records with the
             opt-in block-major/thread-major storage index
             (FRDB_SOA_LAYOUT + FRDB_WARP_CONTIGUOUS_INDEX), bank value
             byte-identical to soa; auto_warp off, transport 3.
...
if not (output / "kernelretsnoop").is_file():
    raise SystemExit(f"missing warp-contiguous-SoA kernelretsnoop binary under {output}")
args = run.parse_args([
...
for block in range(1, 6):
    ...
    for arm in arms[offset:] + arms[:offset]:
        ...
        mode = {"soa-warp": "auto_warp_off", "soa": "auto_warp_off",
                "baseline": "baseline"}[arm]
        tools = {"kernelretsnoop": output if arm == "soa-warp" else soa_dir}
