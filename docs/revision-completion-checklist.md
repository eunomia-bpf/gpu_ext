...
decomposition and bounded raw-map evidence. The abstract/introduction
distinguish policy benefits from measured mechanism costs. The latest source is
pushed as paper commit `db78bb8`. The current
...
| Device trampoline scaling | **Complete:** the repaired [preflight](../microbench/trampoline-scaling/raw/preflight-575-04/summary.md) and [ten-block full campaign](../microbench/trampoline-scaling/results-575-20260903.md) validate all 30 arms / 270 measurements, zero output mismatches, exact independent marker and counter maps, exact target-stub/marker-fallback routing, clean detach, and final UVM refcount zero. At fixed active work, attached absolute time has no increasing trend over 256--4,096 launched blocks. At fixed 4,096-block geometry, the no-op increment remains 0.0012--0.0022 ms while the per-thread counter increment grows from 0.0204 to 0.5417 ms over 2,048--32,768 active warps. | Preserve the boundary: this is a synthetic, verification-disabled runtime measurement. It supports low bounded return-only cost and active-work-dependent policy-body cost, not once-per-warp dispatch or constant cost for arbitrary handlers; audited code uses ordinary per-thread `call`/`call.uni`. |
