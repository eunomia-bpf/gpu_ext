                         ...
                         CUresult *pStatus)
{
    ...
    if (deliver) {
        /* Deliver inside the pre-launch callback after insertion, matching
         * the official mem_trace sample (instrument first, then set the
         * per-launch value consumed by the launch that follows). The outer
         * xg_host_prepare call remains as a fallback arm; this callback
         * delivery is the per-launch source of truth. */
        nvbit_set_at_launch(ctx, f, deliver_val);
    }
}
