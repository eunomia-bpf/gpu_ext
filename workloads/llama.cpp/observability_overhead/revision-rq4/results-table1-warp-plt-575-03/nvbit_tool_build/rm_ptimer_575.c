...
static int raw_ns(uint64_t* value) {
    struct timespec now;
    if (value == NULL || clock_gettime(CLOCK_MONOTONIC_RAW, &now) != 0)
        return -errno;
...
static int xfer(int fd, uint32_t command, void* payload, uint32_t size) {
    ...
    unsigned long request =
        _IOWR(NV_IOCTL_MAGIC, NV_ESC_IOCTL_XFER_CMD, nv_ioctl_xfer_t);
...
static int alloc_object(int fd, uint32_t root, uint32_t parent,
    ...
    if (err) return err;
    if (args.status || !args.hObjectNew) return -EREMOTEIO;
...
static int checked_offset(uint64_t gpu, uint64_t cpu, int64_t padding,
    ...
    if (value < INT64_MIN || value > INT64_MAX) return -ERANGE;
...
static int derive_sample(uint64_t outer_before, uint64_t outer_after,
    ...
    if (sample == NULL || !outer_before || outer_after < outer_before ||
...
int rm_ptimer_575_open(struct rm_ptimer_575_client* client) {
    ...
    if (client->control_fd < 0) return -errno;
...
int rm_ptimer_575_sample(struct rm_ptimer_575_client* client,
    ...
    unsigned long request =
        _IOWR(NV_IOCTL_MAGIC, NV_ESC_RM_CONTROL, nvos54_parameters);
    int err;
    if (client == NULL || client->control_fd < 0 || !client->root ||
        !client->subdevice || sample == NULL)
        return -EINVAL;
    ...
    if ((err = raw_ns(&before))) return err;
    if (ioctl(client->control_fd, request, &control) < 0) return -errno;
    if ((err = raw_ns(&after))) return err;
    if (control.status) return -EREMOTEIO;
    err = derive_sample(before, after, endpoints.cpuBeforeNs,
                        endpoints.gpuTimeNs, endpoints.cpuAfterNs, sample);
    if (!err) sample->rm_status = control.status;
    return err;
}
...
int rm_ptimer_575_self_test(void) {
    ...
    if (derive_sample(1001, 1300, 1000, 2000, 1100, &sample) != -ERANGE ||
        ...
        derive_sample(900, 900 + RM_PTIMER_MAX_OUTER_NS, 1000, 2000, 1100,
                      &sample) != -ERANGE ||
