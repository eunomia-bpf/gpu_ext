...
disk_raw=$disk_repo/workloads/lmcache-disk/raw/diskuvm-serving-fixedmode-20260909.y4NKC1
...
date -Is
kill -INT 452940 452941
disk_loaders_stopped=1
while kill -0 452940 2>/dev/null || kill -0 452941 2>/dev/null; do sleep 0.2; done
while [ "$(</sys/module/nvidia_uvm/refcnt)" != 0 ]; do sleep 0.2; done
rmmod nvidia_uvm
disk_changed=1
...
runuser -u yunwei37 -- "$disk_repo/workloads/lmcache-disk/current-venv/bin/python" -B -u \
    ...
    --blocks 5 --output "/home/yunwei37/workspace/gpu/gpu_ext/workloads/lmcache-disk/raw/diskuvm-serving-fixedmode-20260909.y4NKC1/cells" \
