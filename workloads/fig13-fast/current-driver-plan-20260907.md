...
Next: temporarily load the candidate core, run five rotated four-arm blocks
...
session. No service or module has been changed for this attempt yet.

Local OpenCode session `fig13-scoped-core-lifecycle`, using Qwen Next, owns
the bounded lifecycle script and may not execute it. Root will review and
run it. OpenCode is not given a short timeout and will not be stopped for
