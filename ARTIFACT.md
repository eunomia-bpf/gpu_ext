...
| MoE adaptive byte-admission governor | [Results report](workloads/moe-infinity/results-adaptive-prefetch-575-20260910.md), [CPU audit](scripts/artifact/reanalyze_adaptive.py), [raw records](workloads/moe-infinity/raw/adaptive-prefetch-575/full-20260910-s/) | Five-arm A→B→A campaign: adaptive-native +1.06% vs unbounded ([1.002, 1.020]), 11.4× less logical prefetch traffic, 9.2× less unused-resident bytes; BPF port −1.5% vs native. Throughput and waste reported separately; reduced bytes alone are not claimed as a win. |
...
| MoE adaptive governor (575) | [Lane plan and protocol](workloads/moe-infinity/adaptive-prefetch/plan.md), [runner CLI](workloads/moe-infinity/run_adaptive_prefetch.py) | `dry-run` validates the protocol matrix CPU-only; `preflight` freezes goldens per arm; `run --preflight raw/adaptive-prefetch-575/preflight-20260910-s` executes the five paired blocks bound to that preflight. Verified on a 2026-09-10 detached checkout: `dry-run` output is byte-identical to the measured checkout and [scripts/artifact/reanalyze_adaptive.py](scripts/artifact/reanalyze_adaptive.py) reproduces every statistic from the committed raw subset with no GPU. A real GPU rerun additionally needs the extension rebuilt against a bpftime VM build (published entrypoint: [build_table1_runtime.sh](scripts/artifact/build_table1_runtime.sh), revision `eef8a51a`; feed its `build/` as `BPFTIME_ROOT`/`BPFTIME_BUILD` to `extension/moe_expert_policy.mk`, then run `moe_spec_admission_test` with an absolute `.bin` path) plus the staged 120B snapshot and `.venv` MoE install carried over from the [MoE runtime notes](workloads/moe-infinity/README.md). |
