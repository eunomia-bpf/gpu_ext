...
STAGE4_GUARD_DEVICE_BYTES="${STAGE4_GUARD_DEVICE_BYTES:-1G}"
# Deprecated compatibility alias. It is always allocated as a physical guard.
STAGE4_SAFETY_HEADROOM="${STAGE4_SAFETY_HEADROOM:-${STAGE4_GUARD_DEVICE_BYTES}}"
...
stage4_require_host_memory() {
    local target="$1" ratio="$2" target_bytes planned available gpu_free guard reserve required
...
PY
    ...
    guard="$(bytes_from_size "${STAGE4_GUARD_DEVICE_BYTES}")"
    reserve=$((gpu_free > target_bytes + guard ? gpu_free - target_bytes - guard : 0))
    required=$((planned + reserve + guard + (16 << 30)))
    available="$(( $(awk '/^MemAvailable:/ {print $2}' /proc/meminfo) * 1024 ))"
    ((available > required)) || {
        echo "Host MemAvailable does not cover managed + main reserve + physical guard + 16 GiB." >&2
        return 2
    }
}
