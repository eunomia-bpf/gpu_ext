...
The [four-panel vector plot](figures/fig13_expanded.pdf) and
[PNG preview](figures/fig13_expanded.png) are generated by
[`plot_expanded.py`](plot_expanded.py), using the exact six historical CSVs
listed in that script and the completed 20-row fresh CSV above. Panels
(a)--(c) preserve the original single-round completion-time proxies and
sequential references; panel (d) shows independent high/low process-duration
medians and full min--max ranges across five blocks, not confidence intervals.
All original plot files and data remain unchanged.

Run `python3 workloads/fig13-fast/plot_expanded.py` from the repository root.
The figure is drawn at 7 by 3.5 inches with 7--8 pt fonts and included as a
two-column figure in `docs/paper/tex-revision/tex/eval.tex`. The root inspected
the actual preview and compiled page 12. The revised draft builds successfully
in 17 pages including references, with the conclusion on page 14. Only
`tex-revision` is changed; the frozen `tex` tree is untouched.
