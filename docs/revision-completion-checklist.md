...
| RTX 5090 Table 1 | Independent runtime builds and 41 runner tests pass. [Preflight 07](../workloads/llama.cpp/observability_overhead/revision-rq4/raw/preflight-575-07/summary.md) fixes and passes the kernel-return geometry oracle: both implementations observe exactly 720,896 events across 220 launches, and both thread histograms cover 22,528 nonzero slots. Both launch-latency arms also retain exactly 220 host/device pairs with zero capture, pairing, queue, or clock errors. | Zero timing cells. The preflight remains invalid because the conservative clock-quality gates fail: gpubpf uniquely bins only 23/220 intervals (197 cross a bin boundary), while NVBit uniquely bins 219/220 but its 0.325-second endpoint drift bound is 15,648 ppb, above the frozen 10,000-ppb gate. The failure is retained; timing is blocked pending a principled clock-model correction, not a post-result threshold increase. |
...
| POD-Attention setup decomposition | **Preflight complete; formal campaign running.** The separate [three-arm preflight](../workloads/pod-attention/raw/phase-preflight-575-02/manifest.json) completes inline, original CUDA selection, and BPF selection in fresh processes with the same numerical and device-decision gates. It separates import/setup, first launch, warmup, steady samples, cleanup, and whole-cell time. The preflight is explicitly excluded from formal analysis; no phase-level performance conclusion is yet claimed. |
