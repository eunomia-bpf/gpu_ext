...
/* XG5 consumed-args drain (labeled diagnostic, bounded): the device
 * trampoline's block-0 leader records what it consumed from a publish slot
 * into the slot's unused 4-byte tail (offset 28 of the 32-byte stride).
 * This drains marks for the first 64 slots and logs a slot only when its
 * mark value changed since the previous drain. Intermediate overwrites
 * between drains are not retained. Read-only over
 * the mapped pool; no protocol byte in [0, 28) is touched. */
void InstrumentContext::Xg5Drain(const char *tag)
{
    if (!kToolActuator || tool_ctx_pool_host_ == nullptr) return;
    static std::mutex drain_mtx;
    static uint32_t seen[XG_TOOL_CTX_SLOTS];
    static bool seen_init = false;
    std::lock_guard<std::mutex> lock(drain_mtx);
    if (!seen_init) {
        memset(seen, 0, sizeof(seen));
        seen_init = true;
    }
    const volatile unsigned char *pool =
        (const volatile unsigned char *)tool_ctx_pool_host_;
    for (uint64_t s = 1; s < 64 && s < XG_TOOL_CTX_SLOTS; ++s) {
        const volatile unsigned char *blk = pool + s * XG_TOOL_CTX_STRIDE;
        const uint32_t mark = *(const volatile uint32_t *)(blk + 28);
        if (mark == 0) continue;               /* never consumed (or ordinal 0) */
        if (mark == seen[s]) continue;         /* unchanged since last drain */
        seen[s] = mark;
        const uint32_t type = mark & 3u;
        const uint32_t kidx = (mark >> 2) & 0xFFu;
        const uint32_t has_buf = (mark >> 10) & 1u;
        const uint32_t ord = (mark >> 11) & 0x3FFu;
        const uint32_t flag = (mark >> 21) & 1u;
        const uint32_t pre = (mark >> 22) & 0x1FFu;
        const int64_t pub_kidx = (int64_t)*(const volatile uint64_t *)(blk + 16);
        const uint32_t pub_type = *(const volatile uint32_t *)(blk + 24);
        XINFO("XG5 drain %s slot=" FMT_64U " etype=%u ekidx=%u"
              " ebuf=%u eord=%u eflag=%u epre=%u"
              " pub_kidx=" FMT_64D " pub_type=%u",
              tag, s, type, kidx, has_buf, ord, flag, pre,
              pub_kidx, pub_type);
    }
}
    ...
    *killable    = kernel->killable;
    ...
    if (kToolActuator) {
        ...
        *(uint32_t *)(args + 24) = (uint32_t)type;
        ...
        /* XG5 diagnostic drains at publication counts 24/48/96/192 (one
         * shot each): catch consumptions of earlier slots that happened
         * after the last state-boundary drain (labeled, bounded). */
        {
            static std::atomic<uint64_t> xg5_pub_cnt{0};
            const uint64_t cnt = xg5_pub_cnt.fetch_add(1) + 1;
            if (cnt == 24 || cnt == 48 || cnt == 96 || cnt == 192)
                Xg5Drain("pub");
        }
        launch_mtx_.lock();
        ToolPrepare(kCtx, kernel->kFuncHandle,
...
void InstrumentManager::Xg5Drain(const char *tag)
{
    instrument_ctx_->Xg5Drain(tag);
}
