...
**2026-09-06 update:** Table 1 is measured as llama.cpp pp512 prefill
throughput. The current RTX 5090 campaign completes all seven arms across ten
rotated blocks / 70 numeric cells. Baseline is 37,586.3225 token/s;
gpubpf/NVBit overhead is 90.7051%/99.6210% for `kernelretsnoop`,
...
RTX 5090 Table 1 now has a complete three-tool result. The separate
...
and [S0 campaign](../workloads/llama.cpp/observability_overhead/revision-rq4/device-verifier-s0/results-s0-575-02-20260904.md)
remain scoped verifier studies and do not replace the performance table.
...
| XSched Level-2 matched native route | LC service p99 **2079.29 -> 1195.38 -> 960.35 ms** (baseline -> Level-1 native -> cuXtra Level-2; BPF-host 960.42) | [Five blocks / 20 cells complete](../workloads/xsched/raw/level2-native-route-20260910-082400/README.md). Original cuXtra/SASS Level-2 actuator versus same-source Level-1 and no-policy baseline; moving host HPF from native xserver to BPF (l2_bpfhost) is within 0.4%. GPU-service time, service-only workload; not arrival-to-completion/queueing latency and not the paper's Level-1 launch latency. Supplemental Level-2 evidence. |
| XSched Level-2 policy port | LC GPU-service p99 **2159.61 -> 773.48 -> 892.47 ms**; BE **10.19 -> 9.89 -> 9.90 kernels/s** | [Five blocks / 15 cells complete](../workloads/xsched/raw/level2-device-policy-pair-20260909.buBjns/README.md). Shared NVBit-actuator policy port, native versus BPF guardian decisions. Cuts LC service tail about 64% (native) / 58% (BPF) versus baseline at about 3% lower BE throughput; BPF/native is +13.5% adverse (all five blocks), so parity is not established. Supplemental Level-2, not the paper's Level-1 submission-to-first-block. |
...
The scoped XSched Level-1 comparison is complete; no duplicate rerun is planned. The
...
LMCache was paused at this historical milestone, but that pause was lifted on
2026-09-06; current work is summarized above.
...
| LMCache NVMe | Earlier failed preflights remain retained. The current [five-block result](../workloads/lmcache-disk/results-575-perf-only-five-block-20260906.md) completes 15/15 recompute/CPU/disk cells with output throughput **30.6422/30.1168/28.5723 token/s** and median TTFT **67.1691/72.6468/96.3280 ms**. Native/BPF storage-decision arms remain active work. |
...
| RTX 5090/NVBit | The current [three-tool result](../workloads/llama.cpp/observability_overhead/revision-rq4/results-table1-warp-plt-575-06/README.md) completes **10 blocks / 70 cells**. Baseline is **37,586.3225 token/s**; gpubpf/NVBit overhead is **90.7051%/99.6210%**, **2.9653%/10.3501%**, and **0.2208%/8.7959%** for `kernelretsnoop`, `threadhist`, and `launchlate`. Earlier results and A1/S0 verifier studies remain retained separately. |
...
| R1: scheduling research baseline | XSched on sm_120, explicitly labeled public Level-1 inter-kernel preemption. | Correct epoch/timestamp proof, matched multi-tenant workload, policy engagement, and a predeclared interpretation that permits regressions. | **Complete within public Level-1 scope.** [All 46 cells](../workloads/xsched/performance-full-575-20260903.md) pass: native/original/BPF LC p99 is 76.8780/26.9784/27.2502 s and BE is 10.2377/10.1497/10.1616 kernels/s. This is neither Level-3 nor a formal BPF/original equivalence result. Two supplemental Level-2 sm_120 service-time campaigns (matched native route, shared-actuator policy port) are recorded in the baseline-native-BPF ledger above; they are not the paper's Level-1 launch-latency headline. |
| R1: storage-tier KV baseline | LMCache local-NVMe backend versus CPU retention and recomputation, followed by matched native/BPF storage policy. | Repeated blocks and explicit transport-path labels; direct, compatibility and POSIX executions remain separate. | **Baseline campaign complete, policy comparison active.** Five blocks / 15 cells provide recompute/CPU/disk performance; native/BPF recoverability and GDS decision arms are being implemented. |
...
| R6: current-hardware overhead | RTX 5090 Table 1 comparison including NVBit, plus the gpubpf device-side tools. | Same llama.cpp pp512 prefill benchmark and repeated blocks. | **Complete three-tool campaign.** The [current result](../workloads/llama.cpp/observability_overhead/revision-rq4/results-table1-warp-plt-575-06/README.md) has 70/70 numeric cells and the three overhead pairs reported above. `kernelretsnoop` optimization is a follow-on result, not a prerequisite for retaining this table. |
...
| R8: portability/deployment evidence | Audit the SASS patching prototype, the reported 273 ms one-time ptrace attach, the LD_PRELOAD route, and the approximately 100-LOC open-module patch boundary. | Bind each statement to runnable code or retained raw evidence; otherwise narrow it to design discussion. | The SASS AOT pipeline now demonstrates verified standalone live cubin execution on the RTX 5090 (bpftime `revision/sass-backend` at `fd976ea`): a real clang-built BPF ELF section passes strict GPU verification (explicit 8-byte PREVAIL context plus SIMT), eBPF-to-NVPTX compilation, CUDA 12.9 `ptxas` assembly for `sm_120`, and CUDA Driver API module load, entry-point lookup, 1x1x1 launch, synchronize, and DtoH readback, producing the expected result 42. All CPU build targets and CTest suites passed. The [deployment audit](experiment/revision-deployment-575/RESULTS.md) validates scoped CPU-only `LD_PRELOAD` and running-process lifecycle paths, but does not validate the historical 273-ms number or an unqualified approximately-100-LOC claim. The boundary is standalone generated cubin only; no instrumentation or injection into arbitrary PTX-free existing-application SASS/fatbin, no application hook or helper/map semantics, and not performance evidence or full historical NVBit claim validation. The 610 port remains separate build/portability evidence. |
