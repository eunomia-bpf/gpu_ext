"""vLLM general plugin: scoped UVM allocator pool for model weights.
...
so the ``weights`` tag is backed by the UVM pluggable allocator
(``uvm_allocator.so`` exporting ``uvm_malloc``/``uvm_free``) inside a scoped
``torch.cuda.MemPool``. Every other memory-pool tag (e.g. ``kv_cache``)
delegates to stock vLLM behavior.

Environment (read once, at install time, per process):
    UVM_WEIGHT_PLUGIN
        "1"/"true"/"yes"/"on" enables the plugin. Unset keeps it inert.
    UVM_WEIGHT_PLUGIN_SO
        Absolute path to ``uvm_allocator.so``. Required when enabled.
    UVM_WEIGHT_PLUGIN_COUNTERS
...
logger = logging.getLogger("uvm_weight_plugin")

WEIGHTS_TAG = "weights"
MALLOC_FN = "uvm_malloc"
FREE_FN = "uvm_free"

ENABLE_ENV = "UVM_WEIGHT_PLUGIN"
SO_ENV = "UVM_WEIGHT_PLUGIN_SO"
COUNTERS_ENV = "UVM_WEIGHT_PLUGIN_COUNTERS"
...
def _log_counters(phase: str) -> None:
    try:
        ...
        logger.warning(
            "uvm_weight_plugin: cannot read allocator counters (%s)",
            phase,
            exc_info=True,
        )
        return
    logger.info(
        "uvm_weight_plugin: %s UVM weights pool | allocs=%d frees=%d "
...
@contextmanager
def _uvm_weights_pool():
    """Route allocations through the UVM allocator for the weights context.
    ...
    ``CUDAPluggableAllocator`` wrapper, and the weights stay allocated in the
    pool after the context exits.
    """
...
def install() -> None:
    ...
    if not _env_enabled(ENABLE_ENV):
        logger.debug("uvm_weight_plugin: disabled (set %s to enable)", ENABLE_ENV)
        return

    so = os.environ.get(SO_ENV, "").strip()
    if not os.path.isabs(so):
        logger.error(
            "uvm_weight_plugin: %s must be an absolute path, got %r; staying off",
            SO_ENV,
            so,
        )
        return
    if not os.path.isfile(so):
        logger.error(
            "uvm_weight_plugin: %s does not exist: %s; staying off", SO_ENV, so
        )
        return
    if not torch.cuda.is_available():
        logger.warning("uvm_weight_plugin: CUDA not available; staying off")
        return
    ...
    try:
        ...
        logger.exception("uvm_weight_plugin: cannot wrap GPU worker; staying off")
        return
    if getattr(original, "_uvm_weight_plugin_wrapped", False):
        return

    @functools.wraps(original)
    def wrapped(self, tag: str):
        if tag == WEIGHTS_TAG:
            return _uvm_weights_pool()
        return original(self, tag)

    wrapped._uvm_weight_plugin_wrapped = True
    cls._maybe_get_memory_pool_context = wrapped
    atexit.register(_release_keep_alive)
    logger.info(
        "uvm_weight_plugin: wrapping %s._maybe_get_memory_pool_context; "
