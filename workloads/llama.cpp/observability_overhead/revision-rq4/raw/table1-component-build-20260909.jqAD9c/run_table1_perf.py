...
The opt-in --build-only mode performs the existing gpubpf/NVBit component
preparation (source copy, patch, build) into a new user-selected output
directory, records a compact prepared-tools.json with the resolved tool
paths and mode, and returns before any probe, server, or benchmark; it
refuses an existing nonempty output directory and deletes nothing.
--nvbit-root overrides the NVBit release root for the NVBit tool build
(default: the runner's pinned release under revision-rq4/deps).
"""
...
def dry_run_plan(args: argparse.Namespace) -> dict[str, Any]:
    ...
    return {
        ...
        "nvbit_root": str(args.nvbit_root),
        "nvbit_source_dir": str(runner.NVBIT_SOURCE_DIR),
...
def build_tools(
    ...
    *, tasks: tuple[str, ...] = TASKS, nvbit: bool = True,
    ...
    if nvbit:
        ...
        nvbit_tool = runner.build_nvbit(
            nvbit_build_dir, output_dir,
            nvbit_root=getattr(args, "nvbit_root", None),
        )
...
def run_build_only(args: argparse.Namespace) -> int:
    """Prepare and build the components, then stop before any run.

    Performs the existing gpubpf/NVBit component preparation (source copy,
    patch, build) into a new user-selected output directory and records a
    compact ``prepared-tools.json`` with the resolved tool paths and mode.
    No probe loader, syscall server, or benchmark process is started.
    Refuses an existing nonempty output directory; nothing is deleted.
    """
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_dir = (
        args.output_dir
        or (HERE / "raw" / f"{KIND}-build-{timestamp}")
    ).resolve()
    if output_dir.exists() and any(output_dir.iterdir()):
        raise SystemExit(
            f"build-only: refusing existing nonempty output directory "
            f"{output_dir}; choose a new --output-dir "
    ...
    record: dict[str, Any] = {
        ...
        "nvbit_source_dir": str(runner.NVBIT_SOURCE_DIR),
        "tools": {
            tool: {"source_dir": str(tool_dir), "binary": str(tool_dir / tool)}
    ...
    (output_dir / "prepared-tools.json").write_text(
        json.dumps(record, indent=2) + "\n", encoding="utf-8"
    )
    print(
        f"build-only: built {', '.join(sorted(tool_dirs))}"
        + (" and the NVBit tool" if nvbit_tool is not None else "")
        + f" under {output_dir}; recorded {output_dir / 'prepared-tools.json'}",
        flush=True,
    )
    return 0
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--build-only",
        action="store_true",
        help="prepare and build the gpubpf/NVBit components into a new output "
             "directory, record prepared-tools.json, and stop before any "
             "probe, server, or benchmark",
    )
    parser.add_argument(
        "--nvbit-root",
        type=Path,
        default=None,
        help="NVBit release root for the NVBit tool build; default is the "
             "runner's pinned release under revision-rq4/deps",
    )
    args = parser.parse_args(argv)
    args.pp = PP
    args.tg = TG
    args.nvbit_root = (args.nvbit_root or runner.NVBIT_ROOT).resolve()
