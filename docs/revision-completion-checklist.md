...
| Expert Buffering hot residency | [Fresh Section VI preflight 02](../workloads/expert-buffering-policy/section-vi/correctness-results-575-02.md) passes with the repaired cleanup helper: root reaudits all 27 arrays / 65,636,352 values exactly matching original HF and 20,182 real BPF/native-shadow decisions with zero differences. FIFO/native/BPF exercise real whole-expert copying/eviction at K=16 and the same strict pool; native/BPF final counters agree. The private build, same-worker runner and complete [OpenCode review](../workloads/expert-buffering-policy/section-vi/opencode-review.md) are retained. [Full attempt 01](../workloads/expert-buffering-policy/section-vi/full-01-abandoned.md) was stopped for external CPU interference; all nine completed cells and the interrupted tenth are excluded and preserved. | Five fresh randomized three-arm performance blocks remain. The new CPU guard passes synthetic tests and a real pin/restore canary, but is not itself performance evidence. Untimed correctness is not throughput evidence or a full original distributed-system reproduction; the older page-profile analogue remains separate. |
| Transition-validation pseudocode, SIMT algorithm, rejected policies, failure taxonomy, TCB | Code-grounded exposition is in the draft. [Two actual strict counter pairs pass](../workloads/bpftime-device-smoke/results-strict-575-20260903.md), independently audited: 32,768 callbacks per positive, explicit rejection and zero counters per negative. Scheduler-init shared-validator fixtures pass 28 cases / 705 assertions. The new invalid-prefetch observer builds and passes five synthetic gates, but its [real 575 attempt](experiment/revision-safety/prefetch-invalid-575-01/results.md) fails loading a structure-returning fentry target before releasing the workload; all raw records and clean shutdown observations are retained. | Native scheduler-init and invalid-prefetch transition observations remain open. The failed observer admission is not an invalid99 policy rejection or live fallback test; none of the three functional controls completed. A separately reviewed read-only diagnostic seam is needed before retrying. CPU/compile-only fixtures and the separate strict controls do not retroactively verify performance runs. |
