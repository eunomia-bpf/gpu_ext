#!/usr/bin/env bash
# One recorded invocation; refresh explicit loader PIDs before any reuse.
set -euo pipefail
...
kill -INT 4070246 4070247
hb_stopped=1
systemctl stop gdm nvidia-persistenced
while kill -0 4070246 2>/dev/null || kill -0 4070247 2>/dev/null; do sleep 0.2; done
...
cd "$hb_repo"
for hb_arm in original_inline inline_bpfhost native_adapter; do
    hb_mode=idle_bpf
    ...
    env -u LD_PRELOAD -u CUDA_INJECTION64_PATH \
      ...
      > "$hb_raw/block-00-$hb_arm.log" 2>&1
    printf 'DONE block-00 %s exit=0\n' "$hb_arm"
done
