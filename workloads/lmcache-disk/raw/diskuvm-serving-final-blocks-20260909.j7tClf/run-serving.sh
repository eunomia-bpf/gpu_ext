...
disk_raw=$disk_repo/workloads/lmcache-disk/raw/diskuvm-serving-final-blocks-20260909.j7tClf
...
date -Is
kill -INT 389223 389224
disk_loaders_stopped=1
while kill -0 389223 2>/dev/null || kill -0 389224 2>/dev/null; do sleep 0.2; done
while [ "$(</sys/module/nvidia_uvm/refcnt)" != 0 ]; do sleep 0.2; done
rmmod nvidia_uvm
disk_changed=1
...
runuser -u yunwei37 -- "$disk_repo/workloads/lmcache-disk/current-venv/bin/python" -B -u \
    ...
    --resume --blocks 5 --output "$disk_repo/workloads/lmcache-disk/raw/diskuvm-serving-20260909.9uBMtq/cells" \
