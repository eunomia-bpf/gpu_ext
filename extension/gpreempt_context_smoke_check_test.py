...
class CanaryParserTests(unittest.TestCase):
    def setUp(self):
        ...
        for role in (0, 1):
            ...
            self.rpc.append(dict(event='gsp_timeslice_rpc', pid=7, tid=10 + role,
                                 ...
                                 issue_count=1, wait_count=1, wait_status=0, wait_errors=0,
                                 return_status=0, read_error=0, entered_ns=100, elapsed_ns=10))
        self.rpc.append(dict(event='gpreempt_rpc_observer_summary', entered=2, completed=2,
                             received=2, map_errors=0, ring_drops=0))
    ...
    def test_failure_variants(self):
        for field, bad in [('wait_count', 0), ('issue_count', 0), ('return_status', 5),
                           ('wait_status', 1), ('wait_errors', 1), ('read_error', 1),
                           ('params_size', 4), ('timeslice_us', 200), ('entered_ns', 1100),
                           ('hobject', 77), ('pid', 88)]:
    ...
    def test_later_overwrite_rejected(self):
        ...
        late.update(timeslice_us=3000, entered_ns=500)
        self.rpc.insert(2, late)
        self.rpc[-1].update(entered=3, completed=3, received=3)
        with self.assertRaises(ValueError):
            self.run_case()
