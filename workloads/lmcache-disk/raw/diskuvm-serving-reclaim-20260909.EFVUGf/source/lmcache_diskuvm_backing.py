...
class DiskUvmBacking:
    ...
    def restore(self, dest_device_ptr: int) -> int:
        ...
        A GPU fault over the sealed, on-disk, GPU-promotion range (the driver's
        CPU-staged disk->GPU restore) followed by a device-to-device copy into
        ``dest_device_ptr``.  The range is then re-offloaded: the span is already
        durably on disk, so the driver's ``!write_needed`` path skips the file
        write and unmaps the in-memory copies (CPU + GPU PTEs, ``reclaim_ok``),
        releasing the GPU residency this restore just promoted.  Without that
        reclaim the retained backings can accumulate GPU residency (48 x N-MiB),
        a suspected contributor to the observed engine OOM, to be remeasured.
        The UVM registration and on-disk copy persist, so the range stays
        reusable for the next restore (the next fault re-hydrates from disk or
        from still-resident host pages).  Runs under a per-backing lock so
        concurrent same-key reads do not issue two overlapping re-offloads for
        one range.
        """
        ...
        managed_va = self.backing.managed_va
        uvm_fd = self.backing.uvm_fd
        start = managed_va
        end = managed_va + self.kv_bytes - 1
        with self._restore_lock:
            try:
                self.managed.gpu_fault_read(managed_va, self.kv_bytes)
                return self.managed.copy_to(managed_va, self.kv_bytes,
                                            dest_device_ptr)
            finally:
                # Re-offload to reclaim the in-memory copies (CPU + GPU PTEs) the
                # fault promoted. Runs in a finally to attempt reclamation
                # even when the fault or D2D above raised. A re-offload failure
                # would silently re-accumulate the residency, so the first one is
                # logged, not swallowed.
                try:
                    trace: List[Tuple[int, int]] = []
                    self._offload(uvm_fd, start, end, trace)
                    self._wait_offload_done(uvm_fd, start, end, trace)
                except Exception as cleanup_error:
                    if self.last_cleanup_error is None:
                        self.last_cleanup_error = (
                            f"{type(cleanup_error).__name__}: {cleanup_error}")
                        _log.warning("disk-uvm first re-offload/cleanup failure "
                                     "(pid=%s): %s", os.getpid(), cleanup_error,
                                     exc_info=True)
...
class DiskUvmStore:
    ...
    def record_restore_error(self, error: BaseException) -> None:
        """Remember and log the FIRST restore failure once, so a swallowed
        fallback never hides the real cause (e.g. an engine OOM surfacing as a
        CUDA error on the fault or D2D copy).  Later failures still count; they
        do not re-log, keeping the read path quiet after the first hit."""
        with self._lock:
            if self.restore_error is None:
                self.restore_error = f"{type(error).__name__}: {error}"
                _log.warning("disk-uvm first restore failure: %s", error,
                             exc_info=True)
    ...
    def describe(self) -> Dict[str, Any]:
        with self._lock:
            ...
            for backing in self._backings.values():
                ce = getattr(backing, "last_cleanup_error", None)
                if ce is not None:
                    cleanup_error = ce
                    break
            return {"retained": len(self._backings),
        ...
        if key is not None:
            ...
            if backing is not None and backing.kv_bytes == size_in_bytes:
                ...
                try:
                    ...
                    store.count("error")
                    store.record_restore_error(error)  # fall back to the stock GDS read
...
def backing_diagnostics() -> Dict[str, Any]:
    ...
    taking the stock GDS read.  The first restore/cleanup exceptions are carried
    through verbatim so a swallowed fallback is never mistaken for success."""
    ...
    for store in list(_ALL_STORES):
        ...
        re_err = desc.pop("restore_error", None)
        if re_err is not None and restore_error is None:
            restore_error = re_err
        cl_err = desc.pop("cleanup_error", None)
        if cl_err is not None and cleanup_error is None:
            cleanup_error = cl_err
        for name, value in desc.items():
            aggregate[name] = aggregate.get(name, 0) + int(value)
    aggregate["retained_total"] = retained_total
    if restore_error is not None:
        aggregate["restore_error"] = restore_error
    if cleanup_error is not None:
        aggregate["cleanup_error"] = cleanup_error
    return aggregate
