...
| Expert Buffering hot residency | The historical 20-cell page-profile analogue and [audit](../workloads/expert-buffering-policy/raw-audit.md) remain separate. The [Section VI selector](../workloads/expert-buffering-policy/section-vi/plan.md) and logs are committed (`25be625`): nine CPU tests / 2,131 native and 2,131 actual host-JIT decisions. The additional [adapter state interface](../workloads/expert-buffering-policy/section-vi/adapter-step-a.md) passes 1,432 checks / 650 native-JIT pairs, including forged-status and stale-state rejection. | Live current-batch notification, expert-atomic residency and inactive-first/LIFO eviction adapter remain missing, followed by three-arm correctness and five paired performance blocks. State-interface tests use CPU completion fixtures, not live CUDA/node-lock observations; CPU decision agreement is not a GPU policy reproduction. |
...
| RTX 5090 Table 1 | Independent 575 runtime builds; 24 CPU tests pass. [Three retained preflights](../workloads/llama.cpp/observability_overhead/revision-rq4/preflight-575-results.md): the last tried all seven arms, proves full 1,048,576-slot BPF readback, but rejects four configurations. Every owned cleanup/safety check passes. [Bootstrap-output repair](../workloads/llama.cpp/observability_overhead/revision-rq4/bootstrap-output-repair.md) now passes three actual-header CPU routing checks and a fresh runtime build; its first invalid incremental build is retained. | No timing cells or post-repair GPU checks ran. Verify the repaired BPF stdout on GPU, establish matched event coverage/collection, and repair both launch-latency paths. Do not use the current lower BPF event count as evidence of lower overhead. |
...
| Hummingbird idle scheduling | **Original conservative-port study complete and published (`2658e0e`):** full 50-cell raw comparison and [independent audit](../workloads/hummingbird/raw-audit.md). C/BPF both lose roughly 19–20% background goodput to fixed GPreempt; all results and incomplete-coverage control requests are retained. A [subsequent OpenCode/root fidelity review](experiment/revision-opencode-four-study-audit-20260903.md) identifies the stronger per-piece completion fence as an unresolved confound, not a confirmed cause. An independent fixed-bound pipeline ablation is in source preparation; no new GPU result yet. |
