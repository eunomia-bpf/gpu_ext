...
pushed as paper commit `2c6bbb6`. The last recorded full
[build review](paper/asplos-27-rebuttal/revision-build-review.md), at the earlier
`ee1623e` checkpoint, reports a successful 16-page build with conclusion on
page 14 and no undefined references/citations. A final build and placement
review of all later source additions is still required. Review archive commit
`46d2f70` adds the missing
review updates and meta-review. This is not closure of Table 1, disk
measurements, original agent-log release, or the page-budget check. See
...
| RTX 5090 Table 1 | In retained [preflight 08](../workloads/llama.cpp/observability_overhead/revision-rq4/raw/preflight-575-08/summary.md), both kernel-return implementations observe exactly 720,896 events across 220 launches, but the full comparison remains invalid because gpubpf launch latency classifies only 20/220 intervals. A separate [two-tool preflight](../workloads/llama.cpp/observability_overhead/revision-rq4/raw/preflight-575-noncross-clock-01/summary.md) also remains failed: the pp32 gpubpf kernel-return cell captured 991,232/1,441,792 events and reported all 450,560 capacity drops. The [phase-capacity repair](../workloads/llama.cpp/observability_overhead/revision-rq4/table1-phase-capacity-build-readiness.md) now clean-builds and passes 58 offline tests. | **Incomplete: zero valid formal timing blocks.** Build/readiness and failed preflights are not Table 1 results. A fresh real pp32 preflight must pass before any pp512 full run; launch-latency still needs a principled same-clock/RM correlation path. |
