#!/usr/bin/env python3
"""Minimal performance-only runner: recompute vs lmcache_cpu vs lmcache_disk.

A small companion to run_lmcache_disk.py that measures only warm performance
on the existing Qwen3 workload over five rotated complete blocks (schedule.json
attempts 0-4, every arm exactly once per block) with identical fixed prompts
and server settings in every cell, started through the existing server and
request functions of lmcache_primitives (start_server, wait_ready,
streamed_completion, stop_owned_server, wait_gpu_idle, server_environment).
...
safety admission, retry, or result filtering. Every attempted request, parsed
...
extra wait is one non-gating cold-store barrier (parsing the server log via
request_log_values only) that keeps the warm phase warm without judging
anything and can never abort a cell.
"""
...
def rotation_orders(blocks: int) -> list[list[str]]:
    """Rotated complete block orders from the frozen schedule (attempts 0..blocks-1)."""
    if blocks < 1:
        raise ValueError(f"--blocks must be at least 1, got {blocks}")
    schedule = json.loads(ops.SCHEDULE.read_text())
    orders = [row["order"] for row in schedule["attempts"]]
    if blocks > len(orders):
        raise ValueError(f"schedule has only {len(orders)} rotated attempts, got {blocks}")
    for order in orders[:blocks]:
        if sorted(order) != sorted(CONFIGS):
            raise ValueError(f"schedule order is not a complete three-arm rotation: {order}")
...
def store_barrier(config: str, log_path: Path, engine_request_id: str,
    ...
    while True:
        try:
            ...
            log = ""
            seen["read_error"] = f"{type(error).__name__}: {error}"
...
def run_cell(config: str, block: int, position: int, run_dir: Path, port: int,
    ...
    try:
        try:
            ...
            raise ops.GateError("server log already exists; this cell would not be a first attempt")
        record["command"] = argv
...
def write_summary(root: Path, campaign: dict[str, Any]) -> None:
    lines = [
        ...
        f"- Blocks: `{campaign['params']['blocks']}` (rotated from schedule.json seed "
        f"{campaign['params']['order_seed']})",
...
def run_campaign(args) -> int:
    ...
    campaign: dict[str, Any] = {
        ...
        "params": {
            ...
            "order_seed": json.loads(ops.SCHEDULE.read_text())["order_seed"],
            "configs": list(CONFIGS), "attempts_per_cell": 1,
            "retry": False, "result_filtering": False,
        ...
        try:
            for block, order in enumerate(orders):
                for position, config in enumerate(order):
                    ...
                    run_dir.parent.mkdir(parents=True, exist_ok=False)
...
def dry_run_plan(args) -> dict[str, Any]:
    ...
    return {
        ...
        "reuse": ["start_server", "wait_ready", "streamed_completion", "stop_owned_server",
                  "wait_gpu_idle", "server_environment", "request_log_values (barrier only)",
                  "resolve_model", "file_identity", "atomic_write_json", "shared.Leases"],
        "bypassed_gates": [
