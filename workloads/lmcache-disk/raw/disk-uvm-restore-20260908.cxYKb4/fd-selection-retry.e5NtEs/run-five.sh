#!/usr/bin/env bash
set -euo pipefail
disk_raw=/home/yunwei37/workspace/gpu/gpu_ext/workloads/lmcache-disk/raw/disk-uvm-restore-20260908.cxYKb4/fd-selection-retry.e5NtEs
disk_repo=/home/yunwei37/workspace/gpu/gpu_ext
disk_cache=/var/tmp/disk-uvm-restore-20260908.map3qJ
...
date -Is
kill -INT 3090572 3090573
disk_loaders_stopped=1
while kill -0 3090572 2>/dev/null || kill -0 3090573 2>/dev/null; do sleep 0.2; done
...
for disk_repeat in 01 02 03 04 05; do
    ...
    set +e
    (cd "$disk_raw/repeat-$disk_repeat" && stdbuf -oL -eL "$disk_repo/workloads/lmcache-disk/gds-control/disk-uvm/disk_uvm_perf" --size 256MiB --backing-file "$disk_cache/backing.bin") > "$disk_raw/repeat-$disk_repeat/client.log" 2>&1
    disk_cell_rc=$?
    set -e
