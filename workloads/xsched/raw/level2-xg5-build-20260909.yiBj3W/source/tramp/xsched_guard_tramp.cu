 ...
 *     channel).
 *
 * XG5 consumed-args probe (labeled diagnostic, not performance): the
 * leader thread of block 0 records one u32 mark per launch entry into the
 * unused 4-byte tail of its published 32-byte-stride argument slot
 * (offset 28; the protocol block is 28 bytes and the glue never touches
 * the tail). The mark captures the fields the trampoline actually
 * consumed at CTA entry, not what the host published: bits [1:0]
 * launch_type, [9:2] kernel_idx, [10] preempt_buf != 0, [20:11] a
 * per-entry launch ordinal (device atomic counter), [21] global exit
 * flag word at entry, [30:22] recorded preempt_idx low bits. The host
 * HAL drains changed marks at state boundaries (XG5 drain lines). The
 * probe changes no decision, no record write, and no protocol byte in
 * [0, 28); it stays on in diagnostic builds and is removed with the
 * finding.
 */
#include <stdint.h>
...
/* XG5 consumed-args probe state (diagnostic only). */
static __device__ unsigned long long xg5_ordinal = 0;

static __device__ __forceinline__ void xg5_probe(uint64_t args_dev)
{
    if (threadIdx.x != 0 || threadIdx.y != 0 || threadIdx.z != 0 ||
        blockIdx.x != 0 || blockIdx.y != 0 || blockIdx.z != 0) return;
    const volatile XgArgs *args = (const volatile XgArgs *)args_dev;
    const uint32_t type = (uint32_t)args->launch_type;
    const int64_t kidx = (int64_t)args->kernel_idx;
    const uint64_t buf = (uint64_t)args->preempt_buf;
    uint32_t flag = 0;
    uint32_t pre = 0;
    if (buf != 0) {
        flag = (*(const volatile uint32_t *)buf) != 0 ? 1u : 0u;
        pre = (uint32_t)((*(const volatile unsigned long long *)(buf + 8ULL))
                         & 0x1FFULL);
    }
    const uint32_t ord = (uint32_t)(atomicAdd(&xg5_ordinal, 1ULL) & 0x3FFULL);
    uint32_t mark = ((ord & 0x3FFu) << 11) | ((type & 3u)) |
                    (((uint32_t)(kidx & 0xFF)) << 2) | (buf != 0 ? (1u << 10) : 0u) |
                    (flag << 21) | ((pre & 0x1FFu) << 22);
    *(volatile uint32_t *)(((volatile char *)args_dev) + 28) = mark;
}
