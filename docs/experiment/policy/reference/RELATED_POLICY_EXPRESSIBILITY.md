...
| kernelretsnoop collector timing | No probe -> same ring producer, final-only collection | [`performance`](../../../../workloads/llama.cpp/observability_overhead/revision-rq4/results-final-only-575-20260907/README.md), five baseline/tool pairs | Mean throughput is 3493.318 token/s, paired overhead 90.812%, with all 720896 events per run collected at the end. Changing collector timing alone does not resolve the high overhead. The subsequent GPU-local producer result is separate below; the original Table 1 remains unchanged. |
| kernelretsnoop GPU-local producer | No probe -> full per-warp GPU-array records and one final bulk lookup | [`performance`](../../../../workloads/llama.cpp/observability_overhead/revision-rq4/results-onevalue-array-bootstrap-575-20260907/README.md), five paired blocks / ten successful benchmarks | Mean baseline/tool throughput is 37979.256/35861.535 token/s; mean paired overhead is 5.573%, range 4.018%–6.705%. Every tool run retains 720896 full records. Final lookup averages 10.379 ms, outside prefill timing. This finite pp512 arena uses the existing warning-mode runtime and is not strict-admission or unbounded-streaming evidence. All original numbers remain retained. |
...
now has a completed GPU-local producer follow-up, without removing or replacing
