 ...
 *
 * Callback-delivery repair candidate (20260909), derived from
 * xsched_guard_tool.cu: the per-launch value is now set inside the NVBit
 * pre-launch callback AFTER instrumentation, matching the official NVBit
 * example (mem_trace enter_kernel_launch); the host-side outer
 * nvbit_set_at_launch in xg_host_prepare is kept as a fallback. See
 * callback-delivery-handoff-20260909.md.
 *
