...
#define UPDATE_MAGIC UINT64_C(0x51a7cafe00000001)
#define LOOKUP_MAGIC UINT64_C(0x10c4cafe00000001)

static volatile sig_atomic_t stopping;

static const char *const modes[] = {
	"noop", "device_update", "host_update", "device_lookup", "host_lookup",
};
...
static int prime_bpftime_server(void)
{
	...
	printf("FIG15_UNIFORM_SERVER_PRIMED\t1\n");
	return 0;
}
...
static const char *program_name(const char *mode)
{
	...
	if (strcmp(mode, "host_update") == 0)
		return "cuda__host_up";
	if (strcmp(mode, "device_lookup") == 0)
		return "cuda__dev_look";
	return "cuda__host_look";
}

static const char *source_map_name(const char *mode)
{
	return strncmp(mode, "device_", 7) == 0 ? "device_values" : "host_values";
}

static int write_one(struct bpf_object *object, const char *map_name,
		     uint64_t value)
{
	struct bpf_map *map = bpf_object__find_map_by_name(object, map_name);
	uint32_t key = 0;
	if (!map || bpf_map_update_elem(bpf_map__fd(map), &key, &value, BPF_ANY)) {
...
static int initialize_maps(struct bpf_object *object, const char *mode)
{
	if (!strstr(mode, "_lookup"))
		return 0;
	if (write_one(object, source_map_name(mode), LOOKUP_MAGIC))
		return -1;
	return write_one(object, "observed_values", 0);
}

static int emit_one(struct bpf_object *object, const char *map_name)
{
	struct bpf_map *map = bpf_object__find_map_by_name(object, map_name);
	uint32_t key = 0;
	uint64_t value = 0;
	if (!map || bpf_map_lookup_elem(bpf_map__fd(map), &key, &value)) {
		fprintf(stderr, "failed to read %s: %s\n", map_name, strerror(errno));
		return -1;
	}
	printf("FIG15_UNIFORM_MAP\t%s\t0\t%" PRIu64 "\n", map_name, value);
	return 0;
}

static int emit_maps(struct bpf_object *object, const char *mode)
{
	if (strcmp(mode, "noop") == 0)
		return 0;
	if (strstr(mode, "_update"))
		return emit_one(object, source_map_name(mode));
	if (emit_one(object, source_map_name(mode)))
		return -1;
	return emit_one(object, "observed_values");
}
...
int main(int argc, char **argv)
{
	...
	if (initialize_maps(object, argv[2]))
		goto done;
	...
	printf("FIG15_UNIFORM_READY\t%s\t1\n", argv[2]);
	...
	if (emit_maps(object, argv[2])) {
		bpf_link__destroy(link);
		result = 6;
		goto done;
	}
	bpf_link__destroy(link);
	printf("FIG15_UNIFORM_DETACHED\t1\n");
	result = 0;
