...
## Current execution update — 2026-09-03 02:24 UTC
...
| MoE-Infinity same algorithm | The old generic UVM stride/LFU result (1.63 token/s versus 5.94 UVM and 11.64 MoE) did **not** implement the MoE paper algorithm and is not a same-algorithm comparison. New native/BPF arms share paper-v3 EAMC matching, prefetch ranking, score-based eviction and the serving frontend; BPF decisions execute in the **host ubpf JIT**, not the old kernel-UVM program. The shared executor now includes upstream prediction-set protection and stale-task epochs; the rebuilt store passed [all three new real canaries](../workloads/moe-infinity/raw/paper-v3-575/preflight-protected-849ea75d/README.md). A fresh full campaign is running: **1/5 complete paired blocks independently raw-audited**, with block 2 underway. First-block BPF/native/baseline throughput is 11.0762/11.1562/11.8618 token/s, while first-visible-text TTFT medians are 1518.2/1513.4/1916.8 ms. These are preliminary observations, not a stable performance conclusion. The reboot-interrupted earlier campaign remains at zero valid pairs and is not reused. [Algorithm scope](../workloads/moe-infinity/activation-aware-port.md), [old-result attribution](../workloads/moe-infinity/diagnosis-before-prediction-protection.md). |
