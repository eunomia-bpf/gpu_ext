...
| RTX 5090 Table 1 | Independent runtime builds and 41 runner tests pass. In retained [preflight 08](../workloads/llama.cpp/observability_overhead/revision-rq4/raw/preflight-575-08/result.json), both kernel-return implementations observe exactly 720,896 events across 220 launches, and both thread histograms cover 22,528 nonzero slots. The repaired NVBit launch-latency calibration now passes unchanged quality gates: 214/220 intervals classify, six are uncertain, and endpoint drift is 6,792 ppb. Both launch arms retain all 220 host/device pairs with zero capture, pairing, queue, or clock errors. | Zero timing cells. The overall preflight remains invalid because gpubpf launch latency classifies only 20/220 intervals; 200/220 are uncertain despite its 2,433-ppb endpoint drift. Existing logs cannot separate zero-crossing from bin-crossing uncertainty, so the next step is better diagnostics and a principled same-clock/RM correlation path, not a threshold change. The two non-cross-clock tools are being prepared as an explicitly scoped subset rather than relabelling this failed full preflight. |
