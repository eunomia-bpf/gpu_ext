...
remains invalid. Its [RM/PTIMER recovery canaries](../workloads/llama.cpp/observability_overhead/revision-rq4/launch-clock-recovery/rm-correlation-results.md)
reach the public 575 timer control in 8/8 samples through both xfer and direct
paths, but their 4.759/4.730 us median conservative brackets fail the
predeclared <1.5 us precision gate; this is not completion of the original
three-tool campaign. A separate [verifier-on device plan](../workloads/llama.cpp/observability_overhead/revision-rq4/verifier-on-device-plan.md)
...
| RTX 5090/NVBit | The [accepted two-tool result](../workloads/llama.cpp/observability_overhead/revision-rq4/result-review.md) completes 10/10 blocks on driver 575.57.08: exit records slightly favor NVBit, while exit-count histograms favor gpubpf. `launchlate` remains invalid after [public RM xfer/direct controls](../workloads/llama.cpp/observability_overhead/revision-rq4/launch-clock-recovery/rm-correlation-results.md) both failed the precision gate; verifier-off execution and native-transport boundaries are explicit. The [strict-device follow-up](../workloads/llama.cpp/observability_overhead/revision-rq4/verifier-on-device-plan.md) is planned but has no result yet. |
