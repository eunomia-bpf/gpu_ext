...
| Expert Buffering hot residency | Historical 20-cell page-profile analogue remains separate. [Section VI selector/state](../workloads/expert-buffering-policy/section-vi/plan.md): nine tests / 2,131 native and JIT decisions, plus 1,432 state checks / 650 pairs. The [private adapter](../workloads/expert-buffering-policy/section-vi/adapter-step-b.md) and [actual 19-unit offloader build/import](../workloads/expert-buffering-policy/section-vi/adapter-build-01.md) pass. The [same-worker preflight/full path](../workloads/expert-buffering-policy/section-vi/correctness-plan.md) now passes six shadow and eight controller tests, independently rerun by root; actual checkpoint configuration, 94-entry private inventory and original-HF reference admission also pass. | Three-arm real original-HF/logit correctness, physical residency/eviction/copy engagement, live-input native/JIT parity and five paired performance blocks remain. CPU fixtures and a successful build are not GPU reproduction evidence. OpenCode consultation has not yet supplied a completed review. |
