...
if not root.is_dir():
    raise RuntimeError('resume root missing')
r.ops.atomic_write_text(root / 'resume-run.py', Path(__file__).read_text())
...
r.ops.atomic_write_json(root / 'campaign-before-resume.json', campaign)
campaign['resume_reason'] = 'ENOSPC during block1 native_ratio cold population and result serialization; four block0 cells complete and preserved'
campaign['resume_started_ns'] = time.time_ns()
campaign['resume_old_loader_pid'] = 2054683
campaign['cache_archive'] = '/var/tmp/lmcache-retired-cache-20260908.9ypd5z'
campaign['status'] = 'resuming'
r.ops.atomic_write_json(root / 'campaign.json', campaign)
actual = subprocess.check_output(['sudo', '-n', 'cat', '/proc/2054683/cmdline']).decode().rstrip('\0').split('\0')
if actual != old_command:
    raise RuntimeError('old loader PID no longer matches the owned command; no process changed')
subprocess.run(['sudo', '-n', 'kill', '-TERM', '2054683'], check=True)
while Path('/proc/2054683').exists():
    stat = Path('/proc/2054683/stat').read_text().split()
    if stat[2] == 'Z':
        break
    time.sleep(0.1)
loader = None
try:
    with (root / 'total-cost-loader-resume.log').open('x') as stream:
    ...
    while 'attached' not in (root / 'total-cost-loader-resume.log').read_text():
        if loader.poll() is not None:
            raise RuntimeError(f'total-cost loader exited {loader.returncode}; see total-cost-loader-resume.log')
        time.sleep(0.25)
    ...
    for block in range(1, 5):
        ...
        for position, label in enumerate(order):
            ...
            if block == 1 and position == 0:
                run_dir = run_dir.with_name(run_dir.name + '-after-enospc')
    ...
    if loader is not None and loader.poll() is None:
        ...
        loader.wait()
    with (root / 'restored-original-loader-resume.log').open('x') as stream:
    ...
    while 'attached' not in (root / 'restored-original-loader-resume.log').read_text():
