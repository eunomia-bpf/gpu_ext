"""Mocked CPU-only tests for the UVM kv_cache vLLM general plugin.
...
class PluginTestCase(unittest.TestCase):
    def setUp(self):
        ...
        sys.modules.pop("uvm_kv_plugin", None)
        self.plugin = importlib.import_module("uvm_kv_plugin")
    ...
    def install_enabled(self, so_path: str):
        env = {"UVM_KV_PLUGIN": "1", "UVM_KV_PLUGIN_SO": so_path}
        with mock.patch.dict(os.environ, env):
            self.plugin.install()
        self.assertTrue(
            hasattr(
                self.worker_cls._maybe_get_memory_pool_context,
                "_uvm_kv_plugin_wrapped",
            )
        )
...
class EntryPointTest(PluginTestCase):
    def test_general_plugin_entry_point_declared(self):
        ...
        self.assertEqual(group["uvm_kv_plugin"], "uvm_kv_plugin:install")
...
class GatingTest(PluginTestCase):
    def test_disabled_by_default_delegates(self):
        ...
        self.assertEqual(
            self.worker._maybe_get_memory_pool_context("kv_cache"),
            ("original", "kv_cache"),
        )
        self.assertEqual(self.worker.calls, ["kv_cache"])

    def test_missing_so_env_stays_off(self):
        with mock.patch.dict(os.environ, {"UVM_KV_PLUGIN": "1"}):
            self.plugin.install()
        self.assert_not_wrapped()
    ...
    def test_relative_so_stays_off(self):
        with mock.patch.dict(
            ...
            {
                "UVM_KV_PLUGIN": "1",
                "UVM_KV_PLUGIN_SO": "relative/uvm_allocator.so",
            },
        ):
            self.plugin.install()
        self.assert_not_wrapped()
    ...
    def test_nonexistent_so_stays_off(self):
        with mock.patch.dict(
            ...
            {
                "UVM_KV_PLUGIN": "1",
                "UVM_KV_PLUGIN_SO": "/nonexistent/uvm_allocator.so",
            },
        ):
            self.plugin.install()
        self.assert_not_wrapped()
    ...
    def test_install_is_idempotent(self):
        ...
        with mock.patch.object(self.plugin.atexit, "register") as register:
            with mock.patch.dict(
                os.environ,
                {"UVM_KV_PLUGIN": "1", "UVM_KV_PLUGIN_SO": so},
            ):
                self.plugin.install()
...
class TagRoutingTest(PluginTestCase):
    def test_kv_cache_tag_uses_uvm_pool(self):
        so = self.make_so()
        ...
        ctx = self.worker._maybe_get_memory_pool_context("kv_cache")
        self.assertNotEqual(ctx, ("original", "kv_cache"))
        with ctx:
            pass
    ...
    def test_non_kv_tags_delegate(self):
        so = self.make_so()
        self.install_enabled(so)
        for tag in ("weights", "default", "other"):
            self.assertEqual(
                self.worker._maybe_get_memory_pool_context(tag),
                ("original", tag),
            )
        self.assertEqual(self.worker.calls, ["weights", "default", "other"])
    ...
    def test_pool_objects_kept_alive_then_released(self):
        ...
        with self.worker._maybe_get_memory_pool_context("kv_cache"):
            pass
    ...
    def test_second_kv_cache_entry_creates_new_pool(self):
        so = self.make_so()
        self.install_enabled(so)
        self.pool_cls.return_value = "pool"
