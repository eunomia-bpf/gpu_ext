...
| Expert Buffering hot residency | [The Section VI three-arm GPU preflight](../workloads/expert-buffering-policy/section-vi/correctness-results-575-01.md) now passes and is independently reaudited: all 27 complete logits arrays / 65,636,352 values exactly match original HF; 20,182 real BPF decisions match native shadow with zero differences. FIFO/native/BPF all exercise real whole-expert copying/eviction at K=16 and the same strict pool; native/BPF final counters agree. The tested private build, same-worker runner and complete [OpenCode review](../workloads/expert-buffering-policy/section-vi/opencode-review.md) are retained. The older 20-cell page-profile analogue is separate. | Five complete randomized three-arm performance blocks remain. The successful untimed numerical/shadow preflight is not throughput evidence or a full original distributed-system reproduction. |
