...
public:
    ...
    // Level-2 tool actuator: index of this command's context slot in the
    // InstrumentContext publish pool (slot 0 is reserved, never assigned)
    uint64_t tool_ctx = 0;

    // Level-2 tool actuator diagnostics: read-only accessors over the
    // deep-copied launch parameters (no ownership transfer, no mutation).
    size_t ParamCnt() const { return param_cnt_; }
    const char *ParamData() const { return param_data_; }
